...
public class BallsGenerator {
	...
	private static List<Ball> toList(int ballsNumber) {
		List<Ball> balls = new ArrayList<>();
		balls.add(new Ball(BallGenerator.generateFirstBall(ballsNumber)));
		balls.add(new Ball(BallGenerator.generateSecondBall(ballsNumber)));
		balls.add(new Ball(BallGenerator.generateThirdBall(ballsNumber)));
		return balls;
	}
